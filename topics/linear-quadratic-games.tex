...
\begin{center}
  \begin{tikzpicture}[
    ...
    \node[root node] (root) {Root\\(Level 0)}
    % First level: 3 children (adjust sibling distance above if needed)
    child { node[level-1] (n1) {Node 1.1\\(Level 1)}
      % Second level under Node 1.1: 3 children (leaves)
      child { node[level-2] {Leaf 1.1.1} }
      child { node[level-2] {Leaf 1.1.2} }
      child { node[level-2] {Leaf 1.1.3} }
    }
    child { node[level-1] (n2) {Node 1.2\\(Level 1)}
      % Second level under Node 1.2: Example with ONLY 2 children
      % (comment out one 'child' to reduce)
      child { node[level-2] {Leaf 1.2.1} }
      child { node[level-2] {Leaf 1.2.2} }
      % child { node[level-2] {Leaf 1.2.3} } % <-- Comment this line
      % to make it 2 children
    }
    child { node[level-1] (n3) {Node 1.3\\(Level 1)}
      % Second level under Node 1.3: 3 children (full)
      child { node[level-2] {Leaf 1.3.1} }
      child { node[level-2] {Leaf 1.3.2} }
      child { node[level-2] {Leaf 1.3.3} }
    };
